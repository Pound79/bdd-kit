...
|---|---|---|---|
| `commands.generate` | `.feature` → glue コード生成 | `npm -w e2e run bddgen` | `cd bdd_tests && dart run build_runner build --delete-conflicting-outputs` |
| `commands.typecheck` | 型検査 | `npm -w e2e run typecheck` | `cd bdd_tests && flutter analyze` |
| `commands.lint` | フォーマット + 静的解析 | `npm run lint` | `cd bdd_tests && dart format --set-exit-if-changed . && flutter analyze` |
| `commands.smoke` | 高速サブセット実行（CI 必須パス） | `npm -w e2e run test:smoke` | `cd bdd_tests && flutter test integration_test/gherkin_suite_test.dart -d macos` |
...
|---|---|---|---|
| glue コード生成 | `commands.generate` | `npm -w e2e run bddgen` | `cd bdd_tests && dart run build_runner build --delete-conflicting-outputs` |
| 型検査 | `commands.typecheck` | `npm -w e2e run typecheck` | `cd bdd_tests && flutter analyze` |
| lint | `commands.lint` | `npm run lint` | `cd bdd_tests && dart format --set-exit-if-changed . && flutter analyze` |
| スモークテスト | `commands.smoke` | `npm -w e2e run test:smoke` | `cd bdd_tests && flutter test integration_test/gherkin_suite_test.dart -d macos` |
